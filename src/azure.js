...
api.declare({
  ...
  stability:  'stable',
  scopes:     [['auth:azure-table-access:list-accounts']],
...
api.declare({
  ...
  stability:  'stable',
  scopes:     [['auth:azure-table-access:<account>/list-tables']],
  ...
  if (result.nextTableName) {
    result.continuationToken = result.nextTableName;
  }
  delete result.nextTableName;
